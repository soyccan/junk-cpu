...
RUN sed -ir "s/archive.ubuntu.com/tw.archive.ubuntu.com/g" /etc/apt/sources.list && \
    ...
    apt-get install -yq tmux wget vim git autoconf automake autotools-dev curl \
                        ...
                        libexpat-dev libncurses-dev libncurses5-dev
...
RUN wget -q https://static.dev.sifive.com/dev-tools/riscv64-unknown-elf-gcc-8.3.0-2020.04.1-x86_64-linux-ubuntu14.tar.gz && \
    ...
    mv riscv64-unknown-elf-gcc-8.3.0-2020.04.1-x86_64-linux-ubuntu14 /opt/riscv

# rebuilt gdb for tui
RUN wget ftp://ftp.gnu.org/gnu/gdb/gdb-8.3.1.tar.xz && \
    tar xvf gdb-8.3.1.tar.xz && rm gdb-8.3.1.tar.xz
RUN cd gdb-8.3.1 && \
    ./configure --program-prefix=riscv64-unknown-elf- \
                -with-tui \
